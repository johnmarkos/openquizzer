...
function mcProblem(id, correct = 1) {
  return {
    ...
    question: `MC ${id}`, options: ['A', 'B', 'C', 'D'],
    correct, explanation: `Explanation for ${id}`
  };
}

function numericProblem(id, { answer = 1000, tolerance = 0.1, unit = 'ms' } = {}) {
  return {
    id, type: 'numeric-input',
    question: `Numeric ${id}`, answer, unit, tolerance,
...
function orderingProblem(id) {
  return {
    id, type: 'ordering',
    question: `Order ${id}`, items: ['First', 'Second', 'Third'],
    correctOrder: [0, 1, 2], explanation: `Explanation for ${id}`
  };
}
...
function multiSelectProblem(id, correctIndices = [0, 2]) {
  return {
    ...
    question: `Multi ${id}`, options: ['A', 'B', 'C', 'D'],
    correctIndices, explanation: `Explanation for ${id}`
  };
}
...
function twoStageProblem(id) {
  return {
    id, type: 'two-stage',
    stages: [
      { question: 'Part 1', options: ['X', 'Y'], correct: 0, explanation: 'Stage 1 exp' },
      { question: 'Part 2', options: ['P', 'Q'], correct: 1, explanation: 'Stage 2 exp' }
    ]
  };
}
...
describe('state machine', () => {
  let quiz;
  beforeEach(() => { quiz = new OpenQuizzer(); });
  ...
  it('transitions idle → practicing on start', () => {
    ...
    assert.deepEqual(changes[changes.length - 1], { from: 'idle', to: 'practicing' });
  });
  ...
  it('transitions answered → complete on next when last question', () => {
    ...
    assert.deepEqual(changes[changes.length - 1], { from: 'answered', to: 'complete' });
  });
...
describe('numeric input', () => {
  ...
  it('parses K suffix', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([numericProblem('n1', { answer: 5000, tolerance: 0.01 })]);
    quiz.start();
    const events = collectEvents(quiz, 'numericResult');
  ...
  it('parses M suffix', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([numericProblem('n1', { answer: 2000000, tolerance: 0.01 })]);
    quiz.start();
    const events = collectEvents(quiz, 'numericResult');
  ...
  it('parses commas', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([numericProblem('n1', { answer: 1000000, tolerance: 0.01 })]);
    quiz.start();
    const events = collectEvents(quiz, 'numericResult');
  ...
  it('exact tolerance', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([numericProblem('n1', { answer: 42, tolerance: 'exact' })]);
    quiz.start();
    const events = collectEvents(quiz, 'numericResult');
  ...
  it('order-of-magnitude tolerance', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([numericProblem('n1', { answer: 1000, tolerance: 'order-of-magnitude' })]);
    quiz.start();
    const events = collectEvents(quiz, 'numericResult');
  ...
  it('NaN input is incorrect', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([numericProblem('n1', { answer: 100, tolerance: 'exact' })]);
    quiz.start();
    const events = collectEvents(quiz, 'numericResult');
...
describe('full session flow', () => {
  it('3 questions → score → retry → score', () => {
    const quiz = new OpenQuizzer();
    const problems = [mcProblem('m1', 0), mcProblem('m2', 1), mcProblem('m3', 2)];
...
describe('getters', () => {
  ...
  it('score percentage rounds correctly', () => {
    const quiz = new OpenQuizzer();
    quiz.loadProblems([mcProblem('m1', 0), mcProblem('m2', 0), mcProblem('m3', 0)]);
    quiz.start();

    quiz.selectOption(0); quiz.next();  // correct
    quiz.selectOption(1); quiz.next();  // wrong (correct is 0)
    // 1 of 2 = 50%
